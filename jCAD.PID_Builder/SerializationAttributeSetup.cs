...
namespace jCAD.PID_Builder
{
  ...
  {
    private string Internal_Id { get; set; } = "Internal_Id";
    private void SetRefTextString(AttributeReference attRef, JsonBlockProperty block, BlockTableRecord btr)
    {
      ...
      foreach (var prop in properties)
      {
        ...
        if (customAttributes.Length == 1)
        {
          ...
          if (attRef.Tag == jsonTagName && attRef.Tag != Internal_Id)
          {
    ...
    public JsonBlockProperty SetupAttributeProperty(Transaction tr, BlockReference blockReference, JsonBlockProperty jsonBlockProperty, BlockTableRecord btr)
    {
      ...
      foreach (ObjectId attId in attCol)
      {
        ...
        {
          SetRefTextString(attRef, jsonBlockProperty, btr);

          if (!Wrappers.AlreadyHasAttributeDefined(btr, Internal_Id))
          {
            jsonBlockProperty.Attributes.Internal_Id = Convert.ToString(BlockTableRead.OrderCounter);
          }      
          else
          {
            if (attRef.Tag == "Internal_Id") { jsonBlockProperty.Attributes.Internal_Id = attRef.TextString; continue; }
          }
        }
      }      
